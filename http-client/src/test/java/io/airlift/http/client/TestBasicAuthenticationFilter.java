...
public class TestBasicAuthenticationFilter
{
    @Test
    public void testBasicAuthentication()
            throws Exception
    {
        Predicate<Request> predicate = r -> r.getUri().getPath().startsWith("/private");
        HttpRequestFilter filter = new BasicAuthenticationFilter(predicate, "Aladdin", "open sesame");
    ...
    private Request createTestRequest(String path)
    {
        URI uri = uriBuilderFrom(URI.create("http://example.com"))
                .replacePath(path)
                .build();
        return prepareGet()
                .setUri(uri)
