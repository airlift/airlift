        ...
        implements AsyncHttpClient
{
    ...
    public ApacheAsyncHttpClient(HttpClientConfig config)
    {
        this(config, Collections.<HttpRequestFilter>emptySet());
    }

    public ApacheAsyncHttpClient(HttpClientConfig config, Set<? extends HttpRequestFilter> requestFilters)
    {
    ...
    public void close()
    {
        try {
            ...
            // there is nothing that can be done if this doesn't work
        }
    }
    ...
    @Override
    public <T, E extends Exception> CheckedFuture<T, E> executeAsync(Request request, final ResponseHandler<T, E> responseHandler)
            throws E
    {
