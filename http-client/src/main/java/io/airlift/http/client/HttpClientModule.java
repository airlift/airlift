...
public class HttpClientModule implements Module {
    ...
    public void configure(Binder binder) {
        ...
        binder.bind(HttpClient.class)
                ...
                .toProvider(new HttpClientProvider(name, annotation))
                .in(Scopes.SINGLETON);
    ...
    private static class HttpClientProvider implements Provider<HttpClient> {
        ...
        public HttpClient get() {
            ...
            Optional<SslContextFactory.Client> sslContextFactoryAnnotated =
                    injector.getInstance(Key.get(new TypeLiteral<>() {}, annotation));
            Optional<SslContextFactory.Client> sslContextFactoryGlobal =
                    injector.getInstance(Key.get(new TypeLiteral<>() {}));
            Optional<SslContextFactory.Client> sslContextFactory =
                    sslContextFactoryAnnotated.or(() -> sslContextFactoryGlobal);

            Set<HttpRequestFilter> filters = ImmutableSet.<HttpRequestFilter>builder()
                    .addAll(injector.getInstance(
                            Key.get(new TypeLiteral<Set<HttpRequestFilter>>() {}, GlobalFilter.class)))
            ...
            Set<HttpStatusListener> httpStatusListeners = ImmutableSet.<HttpStatusListener>builder()
                    .addAll(injector.getInstance(
                            Key.get(new TypeLiteral<Set<HttpStatusListener>>() {}, GlobalFilter.class)))
