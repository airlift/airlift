...
public class HttpClientConfig {
    ...
    private List<String> excludedCipherSuites =
            ImmutableList.of("^.*_(MD5|SHA|SHA1)$", "^TLS_RSA_.*$", "^SSL_.*$", "^.*_NULL_.*$", "^.*_anon_.*$");
    ...
    @ConfigDescription(
            "The name of the log file will be prefixed with the name of the HTTP client (<client_name>-http-client.log)")
    ...
    @AssertTrue(
            message =
                    "either both http-client.max-heap-memory and http-client.max-direct-memory are set or none of them")
    ...
    public void validate() {
        if (socksProxy != null && httpProxy != null) {
            throw new ConfigurationException(
                    ImmutableList.of(new Message("Only one proxy can be configured for HttpClient")));
        }
