...
The default exporter protocol is gRPC and its default endpoint is `http://localhost:4317`. When
HTTP/protobuf is selected without an explicit endpoint, its default endpoint is
`http://localhost:4318`.
...
create a second SDK logger provider for application logs. Additional `LogRecordProcessor` bindings
begin receiving records after Guice initialization; records emitted before then are not replayed.
Logs written before Airlift logging is configured remain on the early console path and are not
replayed.
