        ...
        return inputResponses.response("elicitation", ElicitResult.class)
                .map(elicitResult -> {
                    if (elicitResult.action() != ACCEPT) {
                        return new ToolContent(elicitResult.action().toJsonValue());
                    }
                    return (CallToolResult) new ToolContent(elicitResult.content()
                ...
                .orElseGet(() -> {
                    ...
                    return inputRequest("elicitation", new ElicitRequestForm("Who are you?", elicitation));
                });
    }
        ...
        return inputResponses.response("sampling", CreateMessageResult.class)
                .map(createMessageResult -> {
                    if (createMessageResult.stopReason().isPresent()) {
                        return new ToolContent("Stopped: " + createMessageResult.stopReason().orElseThrow().toJsonValue());
                    }

                    if (createMessageResult.content() instanceof TextContent(var text, _)) {
                        return new ToolContent("Response: " + text);
                    }

                    return (CallToolResult) new ToolContent("Response: unknown");
                })
                .orElseGet(() -> inputRequest("sampling", new CreateMessageRequest(Role.USER, new TextContent("Are you sure?"), 100)));
    }

    @McpTool(name = "roots", description = "List roots")
    public CallToolResult listRoots(McpRequestContext requestContext, InputResponses inputResponses)
    {
        return inputResponses.response("roots", ListRootsResult.class)
                .map(roots -> (CallToolResult) new ToolContent(roots.roots().stream()
                        .map(Root::uri)
                        .collect(joining(", "))))
                .orElseGet(() -> inputRequest("roots", new ListRootsRequest()));
    }
}
