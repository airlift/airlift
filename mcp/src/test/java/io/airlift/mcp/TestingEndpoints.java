...
public class TestingEndpoints
{
    ...
    public String elicitation(McpRequestContext requestContext)
    {
        ...
        try {
            response = requestContext.serverToClientRequest(Constants.METHOD_ELICITATION_CREATE, elicitRequest, ElicitResult.class, Duration.ofMinutes(5), Duration.ofSeconds(1));
        }
    ...
    {
        try {
            return requestContext.requestRoots(Duration.ofMinutes(1), Duration.ofSeconds(1))
                    .stream()
                    .map(Root::uri)
                    .collect(joining(", "));
        }
