        ...
        extends TestBase
{
            ...
            throws IOException
    {
        ...
        try (StreamingResponse response = httpClient.executeStreaming(request)) {
            for (Map<String, String> event : readEvents(response.getInputStream())) {
                JsonRpcResponse<Object> jsonRpcResponse = parseData(event, new TypeReference<>() {});
                ListResourcesResult listResourcesResult = jsonRpcResponse.result().map(o -> objectMapper.convertValue(o, ListResourcesResult.class)).orElseThrow();
            ...
            throws IOException
    {
        ...
        try (StreamingResponse response = httpClient.executeStreaming(request)) {
            for (Map<String, String> event : readEvents(response.getInputStream())) {
                JsonRpcResponse<Object> jsonRpcResponse = parseData(event, new TypeReference<>() {});
                ListResourceTemplatesResult templatesResult = jsonRpcResponse.result().map(o -> objectMapper.convertValue(o, ListResourceTemplatesResult.class)).orElseThrow();
