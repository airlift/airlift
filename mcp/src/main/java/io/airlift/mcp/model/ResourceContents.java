...
public record ResourceContents(
        Optional<String> name, String uri, String mimeType, Optional<String> text, Optional<String> blob) {
    public ResourceContents {
        ...
        verify(
                (text.isPresent() || blob.isPresent()) && (text.isPresent() != blob.isPresent()),
                "Only one of text or blob must be present");
    }
