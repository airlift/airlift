...
public record Tool(
        ...
        Optional<ToolExecution> taskSupport)
{
    ...
    public Tool
    {
        ...
        taskSupport = firstNonNull(taskSupport, Optional.empty());
    ...
    {
        return new Tool(name, description, title, inputSchema, outputSchema, annotations, icons, taskSupport);
    }

    public Tool withoutIcons()
    {
        return new Tool(name, description, title, inputSchema, outputSchema, annotations, Optional.empty(), taskSupport);
    }
    ...
    {
        return taskSupport.map(localExecution -> (localExecution == ToolExecution.FORBIDDEN) ? withoutExecution() : this).orElse(this);
    }
