                   ...
                   Meta<ReadResourceResult>
{
    ...
    {
        this(Optional.of(contents), OptionalInt.empty(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty());
    }

    @Override
    public ReadResourceResult withCacheableResult(int ttlMs, CacheScope cacheScope)
    {
        return new ReadResourceResult(contents, OptionalInt.of(ttlMs), Optional.of(cacheScope), requestState, inputRequests, meta);
    }

    @Override
    public ReadResourceResult withMeta(Map<String, Object> meta)
    {
        return new ReadResourceResult(contents, ttlMs, cacheScope, requestState, inputRequests, Optional.of(meta));
    }
}
