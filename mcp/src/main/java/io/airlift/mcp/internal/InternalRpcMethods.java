...
import io.airlift.mcp.session.RequestStateType;
...
public class InternalRpcMethods
{
    ...
    public Response notificationCancelled(@Context Request request, @Context SessionId sessionId)
    {
        Optional<Object> requestId = currentRequestId(request);

        log.debug("Received request cancellation notification. SessionId: %s, RequestId: %s", sessionId, requestId);

        sessionController.ifPresent(controller -> requestId.ifPresent(id -> controller.acceptRequestState(sessionId, id, RequestStateType.CANCELLATION_REQUESTED)));
    ...
    @Produces({SERVER_SENT_EVENTS, APPLICATION_JSON})
    public Response listResources(@Context Request request, @Context SessionId sessionId, @Context Providers providers, @Context ContextResolvers contextResolvers)
    {
        log.debug("Received list resources request. SessionId: %s", sessionId);

        return asStreamingOutput(request, sessionId, Optional::empty,
                notifier -> mcpServer.listResources(new RequestContext(request, sessionId, providers, contextResolvers), notifier));
    }

    @JsonRpc(METHOD_RESOURCES_TEMPLATES_LIST)
    @Produces({SERVER_SENT_EVENTS, APPLICATION_JSON})
    public Response listResourceTemplates(@Context Request request, @Context SessionId sessionId, @Context Providers providers, @Context ContextResolvers contextResolvers)
    {
        log.debug("Received list resources templates request. SessionId: %s", sessionId);

        return asStreamingOutput(request, sessionId, Optional::empty,
                notifier -> mcpServer.listResourceTemplates(new RequestContext(request, sessionId, providers, contextResolvers), notifier));
    }
    ...
    public Map<String, String> subscribeToResource(@Context Request request, @Context SessionId sessionId, SubscribeResourceRequest subscribeResourceRequest)
    {
        ...
        requireSessionController().resourceUpdateSubscription(sessionId, subscribeResourceRequest.uri(), SUBSCRIBE);
    ...
    public Map<String, String> unsubscribeToResource(@Context Request request, @Context SessionId sessionId, SubscribeResourceRequest subscribeResourceRequest)
    {
        ...
        requireSessionController().resourceUpdateSubscription(sessionId, subscribeResourceRequest.uri(), UNSUBSCRIBE);
    ...
    private Response asStreamingOutput(Request request, SessionId sessionId, Meta meta, ResultSupplier resultSupplier)
    {
        ...
        StreamingOutput streamingOutput = output -> {
            try (output) {
                sessionController.ifPresent(controller -> requestId.ifPresent(id -> controller.acceptRequestState(sessionId, id, RequestStateType.STARTED)));
                ...
                try {
                    ...
                    sessionController.ifPresent(controller -> requestId.ifPresent(id -> controller.acceptRequestState(sessionId, id, RequestStateType.ENDED)));
                }
            }
        };
