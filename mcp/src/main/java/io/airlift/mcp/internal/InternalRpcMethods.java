...
public class InternalRpcMethods
{
    ...
    public StreamingOutput callTool(@Context Request request, CallToolRequest callToolRequest)
    {
    ...
    @Produces(SERVER_SENT_EVENTS)
    public StreamingOutput getPrompt(@Context Request request, GetPromptRequest getPromptRequest)
    {
    ...
    @Produces(SERVER_SENT_EVENTS)
    public StreamingOutput listResources(@Context Request request)
    {
        log.debug("Received list resources request");

        return asStreamingOutput(request, Optional::empty,
                notifier -> mcpServer.listResources(request, notifier));
    }

    @JsonRpc(METHOD_RESOURCES_TEMPLATES_LIST)
    @Produces(SERVER_SENT_EVENTS)
    public StreamingOutput listResourceTemplates(@Context Request request)
    {
        log.debug("Received list resources templates request");

        return asStreamingOutput(request, Optional::empty,
                notifier -> mcpServer.listResourceTemplates(request, notifier));
    }

    @JsonRpc(METHOD_READ_RESOURCES)
    @Produces(SERVER_SENT_EVENTS)
    public StreamingOutput readResources(@Context Request request, ReadResourceRequest readResourceRequest)
    {
    ...
    @Produces(SERVER_SENT_EVENTS)
    public StreamingOutput completeCompletion(@Context Request request, CompletionRequest completionRequest)
    {
    ...
    private StreamingOutput asStreamingOutput(Request request, Meta meta, ResultSupplier resultSupplier)
    {
        return output -> {
            try (output) {
