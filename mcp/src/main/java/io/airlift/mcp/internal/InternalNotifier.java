        ...
        implements McpNotifier
{
    ...
    {
        return sessionController.map(controller -> currentRequestId(request)
                .map(requestId -> controller.requestState(sessionId, requestId) == RequestStateType.CANCELLATION_REQUESTED)
                .orElse(false))
                .orElse(false);
    }
