...
import static io.airlift.mcp.model.TaskErrorState.CANCELED;
...
import static io.airlift.mcp.model.TaskErrorState.NONE;
        ...
        implements AutoCloseable, McpTaskController
{
    ...
    @Override
    public Optional<ToolResult> currentTaskResult(String taskId)
    {
        StorageGroupId storageGroupId = toStorageGroupId(taskId);
        Optional<InternalTask> taskEntry = storageController.getValue(storageGroupId, KEY_ID)
                .map(taskEntryCodec::fromJson);
        return taskEntry.map(entry -> {
            ...
            return switch (task.status()) {
                case WORKING, CANCELLED, FAILED -> task.asComplete();
                case COMPLETED -> task.asComplete(entry.result());
                case INPUT_REQUIRED -> task.asInputRequired(entry.result().flatMap(CallToolResult::inputRequests));
            };
        });
    }
    ...
    public InputResponses<?> currentInputResponses(String taskId)
    {
        ...
        return new TaskInputResponses(Optional.empty(), inputResponses);
    }

    @Override
    public TaskErrorState getErrorState(String taskId)
    {
        StorageGroupId storageGroupId = toStorageGroupId(taskId);
    ...
    @Override
    public SetStatus setErrorState(String taskId, TaskErrorState errorState, Optional<JsonRpcErrorDetail> errorDetail)
    {
        ...
        return updateTask(taskId, TASK_NOT_FOUND, (taskEntry, setStatus) -> {
            ...
            checkState((taskEntry.errorState() == TaskErrorState.NONE) || (taskEntry.errorState() == CANCELLATION_REQUESTED), "errorState is already set to %s", taskEntry.errorState());
    ...
    private boolean updateTaskExecution(String taskId, UUID executionId, Consumer<Boolean> taskCancelled)
    {
        return updateTask(taskId, false, (taskEntry, setResult) -> {
            ...
            TaskErrorState errorState = taskEntry.errorState();
