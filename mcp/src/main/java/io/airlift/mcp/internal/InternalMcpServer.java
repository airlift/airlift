...
import static io.airlift.mcp.tasks.TaskCondition.hasMessage;
import static io.airlift.mcp.tasks.TaskCondition.isCompleted;
        ...
        implements McpServer
{
    ...
    JsonRpcMessage blockUntilTaskResult(TaskContextId taskContextId, GetTaskRequest getTaskRequest)
    {
        TaskController localTaskController = requireTaskController();

        try {
            return localTaskController.blockUntilCondition(taskContextId, getTaskRequest.taskId(), pingThreshold, timeout, hasMessage)
                    .message()
    ...
    Task blockUntilTaskCancelled(TaskContextId taskContextId, GetTaskRequest getTaskRequest)
    {
        ...
        try {
            return localTaskController.blockUntilCondition(taskContextId, getTaskRequest.taskId(), pingThreshold, timeout, isCompleted).toTask();
        }
        catch (InterruptedException e) {
            Thread.currentThread().interrupt();
