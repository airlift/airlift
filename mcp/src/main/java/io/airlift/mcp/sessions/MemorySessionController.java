        ...
        implements SessionController
{
    ...
    private final Session systemSession = new Session(new ConcurrentHashMap<>());

    public record Session(Map<ValueKey<?>, Object> values)
    {
        public Session
        {
    ...
    @Override
    public <T> Optional<T> getSessionValue(SessionId sessionId, ValueKey<T> key)
    {
    ...
    @Override
    public <T> boolean setSessionValue(SessionId sessionId, ValueKey<T> key, T value)
    {
        return Optional.ofNullable(sessions.getIfPresent(sessionId))
                .map(session -> {
    ...
    @Override
    public <T> boolean computeSessionValue(SessionId sessionId, ValueKey<T> key, UnaryOperator<Optional<T>> updater)
    {
        return Optional.ofNullable(sessions.getIfPresent(sessionId))
                .map(session -> {
    ...
    @Override
    public <T> boolean deleteSessionValue(SessionId sessionId, ValueKey<T> key)
    {
        return Optional.ofNullable(sessions.getIfPresent(sessionId))
                .map(session -> {
    ...
    @Override
    public <T> Optional<T> getSystemValue(ValueKey<T> key)
    {
        return Optional.ofNullable(systemSession.values().get(key))
                .map(key.type()::cast);
    }

    @Override
    public <T> void computeSystemValue(ValueKey<T> key, UnaryOperator<Optional<T>> updater)
    {
        systemSession.values().compute(key, (_, existingValue) -> {
            Optional<T> existing = Optional.ofNullable(existingValue).map(key.type()::cast);
            return updater.apply(existing).orElse(null);
        });
    }
