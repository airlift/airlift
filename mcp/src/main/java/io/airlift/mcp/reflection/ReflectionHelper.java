    ...
    static List<MethodParameter> parseParameters(Method method)
    {
        return IntStream.range(0, method.getParameterCount())
                .mapToObj(index -> {
                    ...
                    if (!parameter.getType().equals(genericType)) {
                        throw new IllegalArgumentException("Generic objects are not supported: " + parameter);
                    }

                    Optional<String> description = Optional.ofNullable(parameter.getAnnotation(McpDescription.class)).map(McpDescription::value);
                    return new ObjectParameter(parameter.getName(), parameter.getType(), description, Optional.class.isAssignableFrom(parameter.getType()));
                })
