    ...
    public MethodInvoker(Object instance, Method method, List<MethodParameter> parameters, ObjectMapper objectMapper) {
        ...
        try {
            ...
            MethodHandle builder =
                    MethodHandles.lookup().findVirtual(method.getDeclaringClass(), method.getName(), methodType);
                ...
                try {
                    Object[] methodArguments = parameters.stream()
                            .map(parameter -> switch (parameter) {
                                ...
                                case GetPromptRequestParameter _ ->
                                    getPromptRequest.orElseThrow(
                                            () -> new IllegalStateException("GetPromptRequest is required"));
                                case CallToolRequestParameter _ ->
                                    callToolRequest.orElseThrow(
                                            () -> new IllegalStateException("CallToolRequest is required"));
                                case SourceResourceParameter _ ->
                                    sourceResource.orElseThrow(
                                            () -> new IllegalStateException("SourceResource is required"));
                                case ReadResourceRequestParameter _ ->
                                    readResourceRequest.orElseThrow(
                                            () -> new IllegalStateException("ReadResourceRequest is required"));
                                case IdentityParameter _ -> retrieveIdentityValue(request);
                                case ObjectParameter objectParameter ->
                                    valueForObjectParameter(arguments, objectParameter);
                            })
                            .toArray();
