...
public class ResourceHandlerProvider implements Provider<ResourceEntry> {
    ...
    public ResourceHandlerProvider(
            McpResource mcpResource, Class<?> clazz, Method method, List<MethodParameter> parameters) {
        ...
        validate(
                method,
                parameters,
                isHttpRequest.or(isIdentity).or(isReadResourceRequest).or(isSourceResource),
                returnsResourceContents.or(returnsResourceContentsList));
    ...
    static List<ResourceContents> mapResult(Method method, Object result, boolean resultIsSingleContent) {
        if (result == null) {
            throw exception(
                    JsonRpcErrorCode.INTERNAL_ERROR, "ResourceHandler %s returned null".formatted(method.getName()));
        }
    ...
    static Resource buildResource(
        ...
        Optional<String> description =
                descriptionOrEmpty.isEmpty() ? Optional.empty() : Optional.of(descriptionOrEmpty);

        Annotations annotations = new Annotations(
                ImmutableList.copyOf(audience), isNaN(priority) ? OptionalDouble.empty() : OptionalDouble.of(priority));

        OptionalLong useSize = (size >= 0) ? OptionalLong.of(size) : OptionalLong.empty();
        Optional<Annotations> useAnnotations =
                annotations.equals(Annotations.EMPTY) ? Optional.empty() : Optional.of(annotations);
