...
public interface SessionController
{
     ...
     * {@link RequestStateType#STARTED}. If the request is not running on any server
     * or isn't known, the state returned will be {@link RequestStateType#ENDED}. If
     * any client has requested that the request be cancelled, the state returned will be
     * {@link RequestStateType#CANCELLATION_REQUESTED}.
     */
    RequestStateType requestState(SessionId sessionId, Object requestId);

    /**
     * The session controller is responsible for managing the lifecycle of tools, prompts,
     * resources, and other session-related entities. Called when a request state has changed.
     */
    void acceptRequestState(SessionId sessionId, Object requestId, RequestStateType state);
     ...
     */
    void resourceUpdateSubscription(SessionId sessionId, String uri, SubscribeType subscribeType);
