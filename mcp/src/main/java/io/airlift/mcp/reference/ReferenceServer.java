        ...
        implements McpServer
{
    ...
    {
        server.addTool(mapTool(tool, toolHandler));
    }
    ...
    {
        server.addPrompt(mapPrompt(prompt, promptHandler));
    }
    ...
    {
        server.addResource(mapResource(resource, handler));
    }
    ...
    private SyncResourceSpecification mapResource(Resource ourResource, ResourceHandler ourHandler)
    {
        ...
        BiFunction<McpTransportContext, McpSchema.ReadResourceRequest, McpSchema.ReadResourceResult> handler = ((context, theirReadResourceRequest) -> {
            ...
            List<McpSchema.ResourceContents> theirResourceContents = ourResourceContents.stream()
                    .map(this::mapResourceContents)
                    .collect(toImmutableList());

            return new McpSchema.ReadResourceResult(theirResourceContents);
        });

        return new SyncResourceSpecification(theirResourceBuilder.build(), exceptionSafeHandler(handler));
    }

    private SyncPromptSpecification mapPrompt(Prompt ourPrompt, PromptHandler ourHandler)
    {
        List<McpSchema.PromptArgument> ourArguments = ourPrompt
                .arguments()
                .stream()
                .map(this::mapPromptArgument)
                .collect(toImmutableList());
        ...
        BiFunction<McpTransportContext, McpSchema.GetPromptRequest, McpSchema.GetPromptResult> handler = (context, theirGetPromptRequest) -> {
            ...
            List<McpSchema.PromptMessage> promptMessages = promptResult.messages()
                    .stream()
                    .map(this::mapPromptMessage)
                    .collect(toImmutableList());

            return new McpSchema.GetPromptResult(promptResult.description().orElse(null), promptMessages);
        };

        return new SyncPromptSpecification(theirPrompt, exceptionSafeHandler(handler));
    }

    private McpSchema.PromptMessage mapPromptMessage(GetPromptResult.PromptMessage ourPromptMessage)
    {
        return new McpSchema.PromptMessage(mapRole(ourPromptMessage.role()), mapContent(ourPromptMessage.content()));
    }

    private McpSchema.PromptArgument mapPromptArgument(Prompt.Argument ourArgument)
    {
        return new McpSchema.PromptArgument(ourArgument.name(), ourArgument.description().orElse(null), ourArgument.required());
    }

    private SyncToolSpecification mapTool(Tool ourTool, ToolHandler ourHandler)
    {
        try {
            ...
            McpSchema.ToolAnnotations theirToolAnnotations = new McpSchema.ToolAnnotations(
                    ...
                    ourTool.annotations().readOnlyHint().orElse(false),
                    ourTool.annotations().destructiveHint().orElse(false),
                    ourTool.annotations().idempotentHint().orElse(false),
                    ourTool.annotations().openWorldHint().orElse(false),
                    ourTool.annotations().returnDirect().orElse(false));
    ...
    private Map<String, Object> mapStructuredContent(StructuredContent<?> ourStructuredContent)
    {
        return objectMapper.convertValue(ourStructuredContent, new TypeReference<>() {});
    }

    private McpSchema.Content mapContent(Content ourContent)
    {
        return switch (ourContent) {
            case Content.TextContent(var text, var annotations) -> new McpSchema.TextContent(annotations.map(this::mapAnnotations).orElse(null), text);

            case Content.ImageContent(var data, var mimeType, var annotations) ->
                    new McpSchema.ImageContent(annotations.map(this::mapAnnotations).orElse(null), data, mimeType);

            case Content.AudioContent(var data, var mimeType, var annotations) ->
                    new McpSchema.AudioContent(annotations.map(this::mapAnnotations).orElse(null), data, mimeType);

            case Content.ResourceLink(var name, var uri, var description, var mimeType, var size, var annotations) ->
                    new McpSchema.ResourceLink(name, null, uri, description.orElse(null), mimeType, size.orElse(null), annotations.map(this::mapAnnotations).orElse(null), ImmutableMap.of());
            ...
            case Content.EmbeddedResource(var resourceContents, var annotations) -> {
                ...
                yield new McpSchema.EmbeddedResource(annotations.map(this::mapAnnotations).orElse(null), theirResourceContents);
            }
        };
    }

    private McpSchema.ResourceContents mapResourceContents(ResourceContents ourResourceContents)
    {
    ...
    private McpSchema.Annotations mapAnnotations(Annotations ourAnnotations)
    {
        List<McpSchema.Role> audience = ourAnnotations.audience()
                .stream()
                .map(ReferenceServer::mapRole)
                .collect(toImmutableList());
        return new McpSchema.Annotations(audience, ourAnnotations.priority().orElse(0.0));
    }

    private static McpSchema.Role mapRole(Role ourRole)
    {
        return McpSchema.Role.valueOf(ourRole.name().toUpperCase());
    }

    private <T, R> BiFunction<McpTransportContext, T, R> exceptionSafeHandler(BiFunction<McpTransportContext, T, R> handler)
    {
