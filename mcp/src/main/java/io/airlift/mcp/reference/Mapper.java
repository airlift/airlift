...
interface Mapper {
        ...
        return switch (ourContent) {
            ...
            case Content.ResourceLink(var name, var uri, var description, var mimeType, var size, var annotations) ->
                new McpSchema.ResourceLink(
                        ...
                        annotations.map(Mapper::mapAnnotations).orElse(null),
                        ImmutableMap.of());
            ...
            case Content.EmbeddedResource(var resourceContents, var annotations) -> {
                ...
                yield new McpSchema.EmbeddedResource(
                        annotations.map(Mapper::mapAnnotations).orElse(null), theirResourceContents);
            }
        };
    }

    static McpSchema.ResourceContents mapResourceContents(ResourceContents ourResourceContents) {
        return ourResourceContents
                .blob()
                .map(blobValue -> (McpSchema.ResourceContents) new McpSchema.BlobResourceContents(
                        ourResourceContents.uri(), ourResourceContents.mimeType(), blobValue))
                .orElseGet(() -> ourResourceContents
                        .text()
                        .map(textValue -> new McpSchema.TextResourceContents(
                                ourResourceContents.uri(), ourResourceContents.mimeType(), textValue))
                        .orElseGet(() -> new McpSchema.TextResourceContents(
                                ourResourceContents.uri(), ourResourceContents.mimeType(), "")));
    }

    static McpSchema.Annotations mapAnnotations(Annotations ourAnnotations) {
        List<McpSchema.Role> audience =
                ourAnnotations.audience().stream().map(Mapper::mapRole).collect(toImmutableList());
    ...
    static McpStatelessServerFeatures.SyncResourceSpecification mapResource(
            Resource ourResource, ResourceHandler ourHandler) {
        McpSchema.Resource.Builder theirResourceBuilder =
                McpSchema.Resource.builder().name(ourResource.name()).uri(ourResource.uri());
        ...
        BiFunction<McpTransportContext, McpSchema.ReadResourceRequest, McpSchema.ReadResourceResult> handler =
                ((context, theirReadResourceRequest) -> {
                    ...
                    ReadResourceRequest readResourceRequest = new ReadResourceRequest(
                            theirReadResourceRequest.uri(), Optional.ofNullable(theirReadResourceRequest.meta()));

                    List<ResourceContents> ourResourceContents =
                            ourHandler.readResource(request, ourResource, readResourceRequest);
        ...
        return new McpStatelessServerFeatures.SyncResourceSpecification(
                theirResourceBuilder.build(), exceptionSafeHandler(handler));
    }

    static <T, R> BiFunction<McpTransportContext, T, R> exceptionSafeHandler(
            BiFunction<McpTransportContext, T, R> handler) {
    ...
    static McpStatelessServerFeatures.SyncPromptSpecification mapPrompt(Prompt ourPrompt, PromptHandler ourHandler) {
        List<McpSchema.PromptArgument> ourArguments =
                ourPrompt.arguments().stream().map(Mapper::mapPromptArgument).collect(toImmutableList());

        McpSchema.Prompt theirPrompt = new McpSchema.Prompt(
                ourPrompt.name(), null, ourPrompt.description().orElse(null), ourArguments);

        BiFunction<McpTransportContext, McpSchema.GetPromptRequest, McpSchema.GetPromptResult> handler =
                (context, theirGetPromptRequest) -> {
                    HttpServletRequest request = (HttpServletRequest) context.get(McpMetadata.CONTEXT_REQUEST_KEY);
                    GetPromptRequest getPromptRequest = new GetPromptRequest(
                            theirGetPromptRequest.name(),
                            theirGetPromptRequest.arguments(),
                            Optional.ofNullable(theirGetPromptRequest.meta()));

                    GetPromptResult promptResult = ourHandler.getPrompt(request, getPromptRequest);

                    List<McpSchema.PromptMessage> promptMessages = promptResult.messages().stream()
                            .map(Mapper::mapPromptMessage)
                            .collect(toImmutableList());

                    return new McpSchema.GetPromptResult(
                            promptResult.description().orElse(null), promptMessages);
                };
    ...
    static McpSchema.PromptArgument mapPromptArgument(Prompt.Argument ourArgument) {
        return new McpSchema.PromptArgument(
                ourArgument.name(), ourArgument.description().orElse(null), ourArgument.required());
    }

    static McpStatelessServerFeatures.SyncToolSpecification mapTool(
            McpJsonMapper objectMapper, Tool ourTool, ToolHandler ourHandler) {
        try {
            ...
            if (ourTool.outputSchema().isPresent()) {
                theirToolBuilder.outputSchema(
                        objectMapper,
                        objectMapper.writeValueAsString(ourTool.outputSchema().get()));
            }
            ...
            BiFunction<McpTransportContext, McpSchema.CallToolRequest, McpSchema.CallToolResult> callHandler =
                    ((context, theirCallToolRequest) -> {
                        HttpServletRequest request = (HttpServletRequest) context.get(McpMetadata.CONTEXT_REQUEST_KEY);
                        CallToolRequest callToolRequest = new CallToolRequest(
                                theirCallToolRequest.name(),
                                theirCallToolRequest.arguments(),
                                Optional.ofNullable(theirCallToolRequest.meta()));

                        CallToolResult callToolResult = ourHandler.callTool(request, callToolRequest);

                        List<McpSchema.Content> theirContent = callToolResult.content().stream()
                                .map(Mapper::mapContent)
                                .collect(toImmutableList());

                        Map<String, Object> theirStructuredContent = callToolResult
                                .structuredContent()
    ...
    static Map<String, Object> mapStructuredContent(
            McpJsonMapper objectMapper, StructuredContent<?> ourStructuredContent) {
