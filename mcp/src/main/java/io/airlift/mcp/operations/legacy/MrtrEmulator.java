...
public class MrtrEmulator
{
    ...
    <T extends InputRequests<T>> T emulate(LegacyRequestContextImpl requestContext, T result, BiFunction<Optional<String>, Map<String, Object>, T> handler)
    {
        while (result.resultType().orElse(COMPLETE) == INPUT_REQUIRED) {
            ...
            checkArgument(!inputRequests.isEmpty(), "Input requests may not be empty");
