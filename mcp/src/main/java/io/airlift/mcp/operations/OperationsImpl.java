        ...
        implements Operations
{
    ...
    public void handleRpcRequest(HttpServletRequest request, HttpServletResponse response, McpIdentity.Authenticated<?> authenticated, JsonRpcRequest<?> rpcRequest)
    {
        ...
        result = switch (result) {
            ...
            case Task task when method.equals(METHOD_TOOLS_CALL) -> {
                // the extension forbids answering a request that did not declare it with a task
                validateTasksExtension(requestMetadata);
                yield new ResultTypeWrapper(TASK, task);
            }
    ...
    private EmptyResult updateTask(RequestContextImpl requestContext, RequestMetadata requestMetadata, UpdateTaskRequest updateTaskRequest)
    {
        ...
        // the extension requires the field - an answer to nothing is not the same as not answering
        Map<String, Object> inputResponses = updateTaskRequest.inputResponses()
                .orElseThrow(() -> exception(INVALID_PARAMS, "inputResponses is required for " + METHOD_TASKS_UPDATE));

        if (!mcpTasks.updateTask(updateTaskRequest.taskId(), inputResponses)) {
    ...
    private void validateTasksExtension(RequestMetadata requestMetadata)
    {
        requireTasks();
    ...
    private DiscoverResult serverDiscover(RequestContextImpl requestContext, McpMetadata metadata, RequestMetadata requestMetadata)
    {
        ...
        ServerCapabilities serverCapabilities = new ServerCapabilities(
                ...
                tasks.isEmpty() ? Optional.empty() : Optional.of(ImmutableMap.of(EXTENSION_TASKS, ImmutableMap.of())),
                Optional.empty());
