        ...
        implements TaskContext
{
     ...
     *         another attempt is already running is left to it, so the tool runs once at a time.
     */
    private synchronized boolean beginAttempt(TaskExecutor executor)
    {
        if (status.isTerminal() || thread.isPresent()) {
            return false;
        }
        if (cancellationRequested) {
    ...
    private synchronized boolean endAttempt()
    {
        ...
        if (!inputResponsesArrived || status.isTerminal()) {
            return false;
        }
