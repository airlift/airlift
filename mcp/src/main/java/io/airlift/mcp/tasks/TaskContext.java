...
public interface TaskContext
{
     ...
     * request has been answered - {@link #inputResponses} then has the answers. A key that has
     * already been answered cannot be requested again.
     */
     ...
     * were. They accumulate over every round of requests the task has made.
     */
    Map<String, Object> inputResponses();

    /**
     * Asks for input with {@link #requestInput} and blocks until {@code tasks/update} has answered
     * every key or {@code timeout} elapses. Only the answers to {@code inputRequests} are returned;
     * on timeout the unanswered requests are discarded. The task is {@code working} again on return.
     */
