        ...
        implements TaskController, TaskContextController
{
    ...
    @Override
    public TaskAdapter blockUntilResponse(TaskContextId taskContextId, String taskId, Duration timeout, Duration pollInterval)
            throws InterruptedException, TimeoutException
    {
        return blockUntilCondition(taskContextId, taskId, timeout, pollInterval, TaskControllerImpl::hasMessage);
    }

    @Override
    public TaskAdapter blockUntilCompleted(TaskContextId taskContextId, String taskId, Duration timeout, Duration pollInterval)
            throws InterruptedException, TimeoutException
    {
        return blockUntilCondition(taskContextId, taskId, timeout, pollInterval, TaskControllerImpl::isCompleted);
    }
    ...
    public <T> T executeCancellable(TaskContextId taskContextId, String taskId, Supplier<T> supplier)
    {
        return cancellationController.builder(toSessionId(taskContextId), taskAdapterKey(taskId))
                .withTaskId(taskId)
                .withIsCancelledCondition(TaskControllerImpl::cancellationRequested)
    ...
    private TaskAdapter blockUntilCondition(TaskContextId taskContextId, String taskId, Duration timeout, Duration pollInterval, Function<Optional<TaskAdapter>, Boolean> condition)
            throws InterruptedException, TimeoutException
    {
        SessionId sessionId = toSessionId(taskContextId);
        SessionValueKey<TaskAdapter> taskAdapterKey = taskAdapterKey(taskId);
        ...
        while (remainingMs > 0) {
            ...
            sessionController.blockUntilCondition(sessionId, taskAdapterKey, pollInterval, condition);

            TaskAdapter taskAdapter = sessionController.getSessionValue(sessionId, taskAdapterKey)
                    .orElseThrow(() -> exception(INVALID_PARAMS, "Task no longer exists. TaskContextId: %s, TaskId: %s ".formatted(taskContextId, taskId)));
            if (taskAdapter.message().isPresent()) {
                return taskAdapter;
            }
    ...
    private static Boolean cancellationRequested(Optional<TaskAdapter> maybeTask)
    {
        return maybeTask.map(TaskAdapter::cancellationRequested).orElse(false);
    }

    private static Boolean isCompleted(Optional<TaskAdapter> maybeTask)
    {
        return maybeTask.map(task -> task.completedAt().isPresent()).orElse(false);
    }

    private static Boolean hasMessage(Optional<TaskAdapter> maybeTask)
    {
        return maybeTask.map(task -> task.message().isPresent()).orElse(false);
    }
