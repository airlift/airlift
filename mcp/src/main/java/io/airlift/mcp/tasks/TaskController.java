...
public interface TaskController
{
    ...
    /**
     * @return the task for the given task ID, or {@link Optional#empty()} if the task ID is invalid
     */
    Optional<Task> task(TaskId taskId);

    /**
     * Update the status of the given task ID
     *
     * @return {@code true} if the task was updated, {@code false} if the task ID is invalid
     */
    boolean setTaskStatus(TaskId taskId, TaskStatus status, Optional<String> statusMessage);

    /**
     * Set the result of the given task ID. If result is {@code empty()}, the task result is cleared.
     *
     * @return {@code true} if the task was updated, {@code false} if the task ID is invalid
     */
    boolean setTaskResult(TaskId taskId, Optional<CallToolResult> result);
     ...
     */
    Optional<CallToolResult> getTaskResult(TaskId taskId);
     ...
     * be atomic if possible. i.e. each message should be consumed exactly once and then removed from the queue
     ...
     * be atomic if possible. i.e. a response should be consumed exactly once and then removed from the task
