        ...
        implements McpTasks, AutoCloseable
{
    ...
    {
        findTask(taskId).ifPresent(task -> executorService.submit(() -> task.execute(executor)));
    }
        ...
        return findTask(taskId)
                .map(task -> {
                    if (task.deliverInputResponses(inputResponses)) {
                        task.executor().ifPresent(executor -> executorService.submit(() -> task.execute(executor)));
                    }
                    return true;
                })
                .orElse(false);
