...
To create a `TaskContextId` use `createTaskContext()` from the [TaskContextController](src/main/java/io/airlift/mcp/tasks/TaskContextController.java). 
...
You should also periodically call `TaskContextController.validateTaskContext()` for any task contexts you create so that any task context maintenance
can run.
...
inject the [TaskController](src/main/java/io/airlift/mcp/tasks/TaskController.java) and use the `createTask` method, returning the task as part of the `CallToolResult` per the MCP spec. Airlift
manages the lifecycle of the task, but it is your application's responsibility to implement the actual work of the task. Use the methods of
`TaskController` to set server-to-client requests, read responses from the client, etc. Ultimately, when the task is complete, 
call `completeTask` to mark the task as finished.

### Cancellation

Any task related operations should, ideally, be wrapped in `TaskController`'s `executeCancellable()` method so that they can react
