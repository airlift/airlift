...
Airlift comes bundled with an in-memory session implementation. This implmentation only supports
a single server and all session data is lost when the server restarts. For production use cases
a distributed session implementation should be used. See [distributed sessions](distributed-sessions.md) for
documentation on how to create one.
...
- [Session details](sessions.md)
- [Distributed sessions](distributed-sessions.md)