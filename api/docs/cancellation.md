...
decoupling their execution from the main HTTP request thread. Cancelling a request does not
automatically interrupt the running API method or its backend work. Resource code must register an
`ApiCancellation.onCancel(...)` listener that stops or interrupts that work. The listener runs on
the executor supplied when it is registered.
