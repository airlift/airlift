...
public class TestSocketHandler {
    ...
    public void testSocketLogging() throws IOException {
        ...
        try (ServerSocket serverSocket = new ServerSocket(0)) {
            Executors.newSingleThreadExecutor().submit(() -> {
                BufferedHandler handler = createSocketHandler(
            ...
            try (Socket listeningSocket = serverSocket.accept();
                ...
                for (int i = 0; i < lines.length; i++) {
                    ...
                    assertThat(parts[1])
                            .isEqualTo(
                                    io.airlift.log.Level.fromJulLevel(levels[i]).toString());
    ...
    public void testBadListeningSocket() throws IOException {
        ...
        BufferedHandler handler = createSocketHandler(
                HostAndPort.fromParts("localhost", unallocatedPort),
                TEXT.createFormatter(ImmutableMap.of()),
                new ErrorManager());
        ...
        assertThat(((SocketMessageOutput) handler.getMessageOutput()).getFailedConnections())
                .isGreaterThan(0);
    }

    private static BufferedHandler createSocketHandler(
            HostAndPort hostAndPort, Formatter formatter, ErrorManager errorManager) {
