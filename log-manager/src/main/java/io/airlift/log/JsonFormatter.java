...
public class JsonFormatter extends Formatter {
    ...
    private static final JsonCodec<JsonRecord> CODEC =
            new JsonCodecFactory(new ObjectMapperProvider()).jsonCodec(JsonRecord.class);
    ...
    String minimalJsonErrorLogLine(JsonRecord jsonRecord, Exception exception) {
        ...
        try (JsonGenerator jsonGenerator = jsonFactory.createGenerator(stringWriter)) {
            ...
            jsonGenerator.writeStringField(
                    "timestamp", jsonRecord.getTimestamp().toString());
            ...
            // We're using a StringWriter, so all of the operations should be in-memory, and there shouldn't be a way to
            // throw an IOException, but just in case...
