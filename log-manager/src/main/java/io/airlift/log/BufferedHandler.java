...
public class BufferedHandler extends Handler {
    ...
    private final ExecutorService bufferDrainExecutor =
            newSingleThreadExecutor(daemonThreadsNamed("log-buffer-drainer"));
    ...
    // queueDrainLock ensures that queue and dropCountBySource are kept in sync as data is extracted or moved between
    // these structures
                ...
                try {
                    ...
                    try {
                        while (true) {
                            ...
                            // Always check if there are any messages (even if we have a drop snapshot to process) to
                            // ensure that we
                            // are processing at least some messages during overload conditions, and not just only
                            // printing drop summaries.
                            // Note: queuePollFirst() will return null and set terminalMessageDequeued when encountering
                            // terminalMessage.
                            ...
                            if (terminalMessageDequeued) {
                                // Graceful way to exit the drain loop (other than via interruption). The pending
                                // flushedSignal
                    ...
                    // For safety: dropSnapshot and message will be requeued in the finally block if not explicitly
                    // unreferenced
                    ...
                    // Flush after some number of messages or if a flush was requested or if there is nothing more to
                    ...
                    if (message != null || !dropSnapshot.isEmpty()) {
                        ...
                        try {
                            ...
                            if (message != null) {
                                // If terminalMessage has been dequeued, the queue is finished and this message must now
                                // become part of the drop summary.
                                // If dropCountBySource is nonempty, then need to drop the current message because
                                // subsequent messages have already been dropped (regardless of the current queue size).
                                // If the queue is full, then also need to drop the current message. NOTE: this must be
                                // the last condition checked.
                                if (terminalMessageDequeued
                                        || !dropCountBySource.isEmpty()
                    ...
                    if (flushedSignal != null) {
                        // Ensure a flush occurs before notifying. When the terminal message is encountered message
                        // output may not
    ...
    private MessageAndSource queuePollFirst() {
        ...
        if (message.flushSignal != null) {
            ...
            // terminalMessage always contains a flush signal, but flush signals aren't necessarily from the
            // terminalMessage. If
            // this is the terminalMessage, we need to clear other pending flush signals from the queue and notify all
            // of them
            // after completing the terminal flush
            if (message == terminalMessage) {
                terminalMessageDequeued = true;
                SettableFuture<Void> terminalFlushedSignal =
                        requireNonNull(this.flushedSignal, "flushedSignal must be set");
    ...
    private static class LogFormatException extends RuntimeException {
    ...
    public void publish(LogRecord record) {
        try {
            ...
            // Messages may be inserted after being closed, but they won't be processed if they come after the terminal
            // message
            queueInsert(message);
        } catch (LogFormatException e) {
            reportError(null, e, FORMAT_FAILURE);
    ...
    public void flush() {
        ...
        // This technically does not follow traditional buffer flush semantics, but it is not needed for airlift logging
        // uses
        flushMessageOutputSafe();
    }
    ...
    public ListenableFuture<Void> requestFullFlush() {
        if (inputClosed.get()) {
            return nonCancellationPropagating(
                    requireNonNull(terminalMessage.flushSignal(), "terminalMessage flush signal must not be null"));
        }
        ...
        // we enqueued our flush signal before or after termination. If the queue is closed, we'll return the
        // terminalMessage
    ...
    private record MessageAndSource(
            byte[] logMessage,
            String sourceName,
            // Only present on a control message that indicates a flush should occur. The logMessage will be discarded
