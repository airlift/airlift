...
public final class AutomaticMtls {
    ...
    public static X509Certificate addCertificateAndKeyForCurrentNode(
            String sharedSecret, String commonName, KeyStore keyStore, String keyStorePassword) {
        try {
    ...
    @VisibleForTesting
    static X509Certificate addCertificateToKeyStore(
            String sharedSecret,
            String commonName,
    ...
    public static void addClientTrust(String sharedSecret, KeyStore keyStore, String commonName) {
        try {
            X509Certificate certificateServer =
                    certificateBuilder(sharedSecret, commonName).buildSelfSigned();
    ...
    public static SSLContext createSSLContext(
            String sharedSecret, String commonName, KeyStore keyStore, String keyManagerPassword) {
        try {
            ...
            context.init(
                    ...
                    new TrustManager[] {createTrustManager(sharedSecret, commonName)},
                    null);
            return context;
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    private record SingleCertificateTrustManager(PublicKey trustedPublicKey, String commonName)
        ...
        private void check(X509Certificate[] chain) {
            ...
            if (!peerSubject.equals(expectedSubject)) {
                throw new SecurityException("Peer certificate subject '%s' does not match expected subject: '%s'"
                        .formatted(peerSubject, expectedSubject));
            }

            X500Principal peerIssuer = certificate.getIssuerX500Principal();
            if (!peerIssuer.equals(expectedSubject)) {
                throw new SecurityException("Peer certificate issuer '%s' does not match expected subject: '%s'"
                        .formatted(peerIssuer, expectedSubject));
            }

            Instant now = Instant.now();
            if (certificate.getNotBefore().toInstant().isAfter(now)) {
                throw new SecurityException(
                        "Peer certificate is not valid yet (notBefore: %s)".formatted(certificate.getNotBefore()));
            }

            if (certificate.getNotAfter().toInstant().isBefore(now)) {
                throw new SecurityException(
                        "Peer certificate has expired (notAfter: %s)".formatted(certificate.getNotAfter()));
            }
        }
