...
public class LabeledHistogramStat
        extends AbstractLabeledStat<LabeledHistogramStat.HistogramStat>
{
    ...
    @Override
    protected HistogramStat createStat(LabelSet labels)
    {
        return new HistogramStat(metricName, description, labels, buckets);
    }

    public void observe(LabelSet labels, double value)
    {
        getOrCreate(labels).observe(value);
    }

    public void batchObserve(LabelSet labels, double value, long count)
    {
        checkArgument(count >= 0, "count must be non-negative");
        getOrCreate(labels).batchObserve(value, count);
    }
     ...
     */
    @ThreadSafe
    public static class HistogramStat
            extends LabeledStat
    {
        ...
        public HistogramStat(String metricName, String description, LabelSet labels, double[] buckets)
        {
