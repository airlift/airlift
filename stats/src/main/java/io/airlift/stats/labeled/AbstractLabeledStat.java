...
public abstract class AbstractLabeledStat<T extends AbstractLabeledStat.LabeledStat>
{
    ...
    protected T getOrCreate(LabelSet labelSet)
    {
        ...
        // cardinality check is not synchronized, accept races that allow cardinality limit to be mildly exceeded
        if (labeledStats.size() >= labeledStatRegistry.getLabeledStatMaxCardinality()) {
        ...
        return labeledStats.computeIfAbsent(labelSet, _ -> {
            T stat = createStat(labelSet);
            try {
                labeledStatRegistry.getMBeanExporter().export(objectNameGenerator.generatedNameOf(labelSet.asMap()), stat);
            }
            catch (Exception e) {
                log.error(e, "Failed to register MBean for labels %s", labelSet);
            }
            return stat;
        });
    }
