...
public final class DecayConfig
{
    ...
    // all no-op tickers are functionally identical (always 0), so share a single instance
    private static final Ticker NO_OP_TICKER = new Ticker()
    {
        @Override
        public long read()
        {
    ...
    /**
     * A config with no decay (every entry keeps a weight of 1 forever).
     */
    public static DecayConfig all()
    {
        return new DecayConfig(0, systemTicker());
    }

    /**
     * A config with no decay, bound to the given ticker.
     */
    public static DecayConfig all(Ticker ticker)
    {
        return new DecayConfig(0, ticker);
    }
    ...
    private DecayConfig(double alpha, Ticker ticker)
    {
        checkArgument(alpha >= 0 && alpha < 1, "alpha must be in range [0, 1)");
        requireNonNull(ticker, "ticker is null");
        this.alpha = alpha;
        // With no decay every weight is 1 regardless of the landmark, so avoid reading the clock at all
        this.ticker = alpha == 0.0 ? NO_OP_TICKER : ticker;
    }
    ...
    {
        return alpha > 0.0 && ageInSeconds >= RESCALE_THRESHOLD_SECONDS;
    }
