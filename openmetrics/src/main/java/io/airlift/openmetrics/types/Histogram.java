        ...
        implements Metric
{
    ...
    public static Histogram fromLabeledHistogramStat(Map<String, String> commonLabels, LabeledHistogramStat.Value labeledHistogramStat)
    {
        LabelSet mergedLabels = LabelSet.fromLabels(commonLabels, labeledHistogramStat.labels().asMap());
        return new Histogram(
                ...
                mergedLabels,
                labeledHistogramStat.description());
    }
    ...
    public String getMetricExposition()
    {
        ...
        for (int i = 0; i < bucketBounds.length; i++) {
            ...
            sb.append(Metric.formatSingleValuedMetric(metricBucketPrefix, bucketLabels, String.valueOf(cumulativeCount)));
        }
        sb.append(Metric.formatSingleValuedMetric(metricBucketPrefix, generateBucketLabels("+Inf"), String.valueOf(count)));

        sb.append(Metric.formatSingleValuedMetric(metricName + "_count", labels.asMap(), String.valueOf(count)));
        sb.append(Metric.formatSingleValuedMetric(metricName + "_sum", labels.asMap(), String.valueOf(sum)));

        if (created.isPresent()) {
            sb.append(Metric.formatSingleValuedMetric(metricName + "_created", labels.asMap(), String.valueOf(created.orElseThrow())));
        }
    ...
    {
        return Metric.formatMetricDescriptor(metricName, "histogram", help);
    }
