        ...
        implements McpTasksConnection
{
    ...
    public static InternalConnection createInternalConnection(McpClient client, URI uri, SettingContainer settingContainer, boolean tasksEnabled)
    {
        ...
        ThreadFactory threadFactory = virtualThreadsNamed(uri.toString());
    ...
    {
        return serverDiscover(new MetaOnly(Optional.empty()));
    }

    @Override
    public DiscoverResult serverDiscover(Meta<?> meta)
    {
        MetaOnly request = new MetaOnly(meta.meta());
    ...
    private <T, R extends CacheableResult<?>> R sendCacheableRequest(String method, T request, Class<R> responseType)
    {
        ...
        Optional<String> cursor = (request instanceof PaginatedRequest paginatedRequest) ? paginatedRequest.cursor() : Optional.empty();
        RequestCache requestCache = requestController.settingContainer().getSettingValue(REQUEST_CACHE);
        return requestCache.executeRequest(httpRequest, method, request, responseType, cursor, () -> requestController.sendRequest(httpRequest, responseType).result());
    }
