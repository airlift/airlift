...
public class ConfigBinder {
    ...
    private static final class GuiceConfigBinder implements InternalConfigBinder {
        ...
        public <T> void bindConfigDefaults(ConfigDefaultsHolder<T> configDefaultsHolder) {
            createConfigDefaultsBinder(configDefaultsHolder.getConfigKey())
                    .addBinding()
                    .toInstance(configDefaultsHolder);
        }
        ...
        public void bindConfigurationBindingListener(ConfigurationBindingListener configurationBindingListener) {
            listenerMultibinder
                    .addBinding()
                    .toInstance(new ConfigurationBindingListenerHolder(configurationBindingListener));
        }
        ...
        private static <T> TypeLiteral<ConfigDefaultsHolder<T>> getTypeLiteral(Key<T> key) {
            TypeToken<T> typeToken =
                    (TypeToken<T>) TypeToken.of(key.getTypeLiteral().getType());
            return (TypeLiteral<ConfigDefaultsHolder<T>>) TypeLiteral.get(
                    new TypeToken<ConfigDefaultsHolder<T>>() {}.where(new TypeParameter<T>() {}, typeToken)
                            .getType());
        }
    }

    private static final class CallbackConfigBinder implements InternalConfigBinder {
        ...
        @Override
        public <T> void bind(ConfigurationBinding<T> configurationBinding) {
            configurationFactory.registerConfigurationProvider(
                    new ConfigurationProvider<>(configurationBinding), bindingSource);
        }
    ...
    public <T> void bindConfigDefaults(
