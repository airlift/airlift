...
public class TestConfigurationFactory
{
    ...
    public void testEnumWithFromString()
    {
        ...
        assertThat(injector.getInstance(EnumWithFromStringClass.class).value).isSameAs(EnumWithFromStringClass.Value.TRUE);
        monitor.assertNumberOfErrors(0);
        monitor.assertNumberOfWarnings(0);
    ...
    private static void assertMessagesMatch(Collection<? extends Object> messages, List<String> expectedErrorMessagePatterns)
    {
        assertThat(messages.size())
                .isEqualTo(expectedErrorMessagePatterns.size())
                .describedAs("expected error count");
            ...
            for (Pattern pattern : patterns) {
                if (pattern.matcher(messageString).matches()) {
                    assertThat(matchedPattern)
                            .describedAs(format("Error message matches two patterns patterns:\nmessage:\n  %s\npatterns:\n  %s\n  %s", messageString, matchedPattern, pattern))
                            .isNull();
                    String usedMatch = usedPatterns.put(pattern, messageString);
                    assertThat(usedMatch)
                            .describedAs(format("Pattern '%s' matches message '%s' and '%s", pattern, messageString, usedMatch))
                            .isNull();
                    matchedPattern = pattern;
                }
            }
            assertThat(matchedPattern)
                    .describedAs(format("Error message did not match any expected patterns:\nmessage:\n  %s\npatterns:\n  %s", messageString, Joiner.on("\n  ").join(patterns)))
                    .isNotNull();
        }
    }
