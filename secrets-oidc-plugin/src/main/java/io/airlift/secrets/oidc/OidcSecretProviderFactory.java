...
public class OidcSecretProviderFactory
        implements SecretProviderFactory
{
    ...
    public SecretProvider createSecretProvider(Map<String, String> config)
    {
        Bootstrap app = new Bootstrap(
                binder -> {
                    configBinder(binder).bindConfig(OidcSecretProviderConfig.class);
                    httpClientBinder(binder).bindHttpClient("client", ForOidcHttpClient.class);
                    binder.bind(SecretProvider.class).to(OidcSecretProvider.class).in(Scopes.SINGLETON);
                });
