...
public final class Announcer {
    ...
    public void destroy() {
        ...
        try {
            ...
            if (e.getCause() instanceof ConnectException) {
                log.error(
                        "Cannot connect to discovery server for unannounce: %s",
    ...
    private ListenableFuture<Duration> announce(long delayStart, Duration expectedDelay) {
        ...
        if (System.nanoTime() - (delayStart + expectedDelay.roundTo(NANOSECONDS)) > SECONDS.toNanos(5)) {
            log.error(
                    "Expected service announcement after %s, but announcement was delayed %s",
                    expectedDelay, Duration.nanosSince(delayStart));
        }
