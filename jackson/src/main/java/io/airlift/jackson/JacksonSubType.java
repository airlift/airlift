            ...
            extends JacksonAnnotationIntrospector
    {
        ...
        public Value findPolymorphicTypeInfo(MapperConfig<?> config, Annotated annotated)
        {
            ...
            if (subClassPropertyValues.containsKey(rawType)) {
                ...
                if (!(annotated instanceof AnnotatedClass) || declaresTypeInfoBelowBase(config, rawType)) {
        ...
        private boolean declaresTypeInfoBelowBase(MapperConfig<?> config, Class<?> type)
        {
            // every declaration Jackson would see for the subtype counts as its own except the registry base's;
            // stop at the base so its declaration and its ancestors' are never taken for the subtype's
            if (type == null || type.equals(baseClass) || type.isAssignableFrom(baseClass)) {
                return false;
            }
            // a declaration written on the type, or supplied for it through a mix-in, is the type's own
            Class<?> mixIn = config.findMixInClassFor(type);
            if (type.getDeclaredAnnotation(JsonTypeInfo.class) != null || (mixIn != null && mixIn.getDeclaredAnnotation(JsonTypeInfo.class) != null)) {
                return true;
            }
            if (declaresTypeInfoBelowBase(config, type.getSuperclass())) {
                return true;
            }
            return Arrays.stream(type.getInterfaces()).anyMatch(candidate -> declaresTypeInfoBelowBase(config, candidate));
        }
